...
qbt::MarketSnapshot market_bar(const char* symbol, qbt::Timestamp timestamp,
                               double close) {
  ...
  value.lot_size = 1;
  value.industry = "phase1a_reference";
  return value;
}
...
performance_analytics::PerformanceSpecV1 performance_spec() {
  ...
  value.calendar_id = "XSHG_PHASE1A_REPLAY_V1";
  value.calendar_periods_per_year = 242.0;
  value.benchmark_id = "PHASE1A_REFERENCE";
  value.config_hash = 2'026'080'800;
  return value;
}
...
std::vector<qbt::Order> build_orders(
  ...
  const std::array<engine_common::SymbolId, 3> symbols{0, 1, 2};
  const std::array<std::string, 3> names{"AAA", "BBB", "CCC"};
  std::array<engine_common::MarketBar, 3> market{};
  std::array<engine_common::PortfolioItem, 3> portfolio{};
  std::array<double, 3> current_weights{};
  ...
  for (std::size_t index = 0; index < batch.size(); ++index) {
    ...
    market[index] = {
        ...
        value.volume, value.lot_size,
        engine_common::MARKET_LISTED | engine_common::MARKET_DATA_TRUSTED};
    const auto position = engine.get_position(value.symbol);
    portfolio[index] = {symbols[index], position.quantity,
                        position.sellable_quantity, position.avg_cost,
                        value.close, 0, 0};
  ...
  reconciler_options.reconciler_spec_hash = 2'026'080'801;
  reconciler_options.costs_available = true;
  const std::array<double, 3> penalty{1.0, 1.0, 1.0};
  const std::array<double, 3> linear_cost{0.001, 0.001, 0.001};
  const std::array<double, 3> quadratic_impact{0.01, 0.01, 0.01};
  const auto reconciled = portfolio_math::reconcile_single_period(
      anchor_weights, current_weights, penalty, linear_cost,
      quadratic_impact, reconciler_options);
  if (reconciled.diagnostics.status != portfolio_math::OptimizationStatus::OK) {
    return {};
  }
  std::array<engine_common::OrderIntent, 8> intent_storage{};
...
bool run_replay(std::span<const double> weights, std::uint64_t config_hash) {
  ...
  engine.set_on_cross_section([&](
    ...
    return build_orders(engine, batch, timestamp, weights);
  });
  const std::array<std::array<double, 3>, 6> prices{{
      {10.0, 20.0, 30.0}, {10.5, 19.8, 30.4}, {11.0, 20.2, 30.1},
      {10.8, 20.5, 30.6}, {11.3, 19.9, 30.2}, {11.0, 20.1, 30.8}}};
  const std::array<std::string, 3> names{"AAA", "BBB", "CCC"};
  for (std::size_t index = 0; index < prices.size(); ++index) {
    ...
    engine.process_market_data_batch({
        market_bar(names[0].c_str(), timestamp, prices[index][0]),
        market_bar(names[1].c_str(), timestamp, prices[index][1]),
        market_bar(names[2].c_str(), timestamp, prices[index][2])});
    if (index == 0) {
  ...
  bool ok = check(!sink->failed() && sink->return_ledger().records().size() == 5,
                  "phase1a replay ledger");
  ok &= check(engine.get_order_count() > 0 && engine.get_trade_count() > 0,
              "phase1a replay order/fill");
  ...
  ok &= check(std::all_of(records.begin(), records.end(), [](const auto& record) {
              ...
              "phase1a replay accounting residual");
  ...
  tail_spec.config_hash = config_hash;
...
int main() {
  ...
  bool ok = check(comparison.all_estimators_succeeded,
                  "phase1a estimator closure");
  const auto sample = std::array<double, 3>{0.5, 0.5, 0.0};
  ok &= run_replay(sample, 2'026'080'802);
  for (const auto* selection : {
           &comparison.sample,
           &comparison.linear_constant_correlation,
           &comparison.nonlinear_minimum_variance_quest}) {
    ...
    const auto risk_budget = portfolio_math::solve_long_only_risk_budget(
        quant_math::view(selection->covariance.covariance), budgets, {}, {}, options);
    ok &= check(risk_budget.diagnostics.status ==
                    portfolio_math::OptimizationStatus::OK,
                "phase1a risk-budget estimator switch");
    ok &= run_replay(risk_budget.weights, 2'026'080'803);
  }
  if (!ok) return 1;
  std::printf("test_phase1a_replay: all checks passed\n");
  return 0;
}
