...
- direction 与 volatility FFV 已有 reference：volatility 采用固定中心的二阶矩约束，目标波动率在
  solver 内平方为目标方差；尚无 ranking/quantile view、完整 active-set adapter、BL-vs-FFV Posterior Direct
...
下一步按路线图补 ranking/quantile view 的独立 oracle，完成 inequality active-set adapter，再用已固定的
