...
研究 artifact 已有数值输出；当前未形成正式 challenger 晋级，原因是：

1. C++ Replay/CVaR 使用 bar-close/reference proxy，费用、涨跌停、公司行动、lot、滑点和正式
   reference-price provenance 仍缺失，因此所有经济 artifact 强制 `promotion_eligible=false`；这不再
   阻塞 `RESEARCH_PROXY` 阶段施工，但生产经济晋级延后到实盘数据接入。
2. PCGrad 和 GradNorm-4 均存在 NDCG/precision/turnover 等退化项，未通过研究 challenger 的主指标
   非劣门槛；`legacy + fixed` 继续冻结为冠军。
3. e4 复跑已补齐 Phase 1D 的完整六输出 prediction artifact 和 embedding snapshot；Phase 1D
   `BAR_V1` feature pipeline 已链接并完成研究工程退出，生产经济晋级仍保持 `promotion_eligible=false`。
...
- `/Users/Zhuanz/CLionProjects/quant-backtester-cpp/runs/cpp-proxy/pcgrad-fold-1.json`
- `/Users/Zhuanz/CLionProjects/quant-backtester-cpp/runs/cpp-proxy/pcgrad-fold-2.json`
- `/Users/Zhuanz/CLionProjects/quant-backtester-cpp/runs/cpp-proxy/pcgrad-fold-3.json`
- `/Users/Zhuanz/CLionProjects/quant-backtester-cpp/runs/cpp-proxy/gradnorm-fold-1.json`
- `/Users/Zhuanz/CLionProjects/quant-backtester-cpp/runs/cpp-proxy/gradnorm-fold-2.json`
- `/Users/Zhuanz/CLionProjects/quant-backtester-cpp/runs/cpp-proxy/gradnorm-fold-3.json`
...
| 生产费用/涨跌停/公司行动/reference gate | DEFERRED_TO_LIVE，RESEARCH_PROXY 可用 |
| Phase 1D real drift baseline/report | PASS，RESEARCH_PROXY |
| Phase 1D 六输出/embedding 内部产物 | PASS，六输出/embedding/manifest/CKA 可复核 |
| 自动晋级/覆盖 `FROZEN_CHAMPION` | NO-GO |

## 9. e4 内部产物闭环

- PCGrad 与 GradNorm-4 各 3 个 fold 均生成 validation/test 六输出；每个 snapshot 维度为 `64`，
  manifest、prediction hash、anchor hash 校验通过，anchor 全部为有限值。
- PCGrad 报告：`runs/phase1e-pcgrad-real-e4/challenger_report.json`，SHA-256：
  `ddbdaef8f37f419847a9fec7041b1cd6e8349d45c3e7833945949f6b87fb448a`。
- GradNorm-4 报告：`runs/phase1e-gradnorm4-real-e4/challenger_report.json`，SHA-256：
  `ca1e20ea5b521464e6a82d27443df602835ad188237546964356803c2c17bd5b`。
- Phase 1D fold-1 基线报告已消费 PCGrad e4 validation/test artifact；23 个 `BAR_V1` features、六个
  heads 均为 available，embedding status=`OK`，linear CKA=`1.0`，`phase_exit_eligible=true`。

最终决策：**Phase 1E 研究施工与内部产物闭环完成；正式生产经济晋级和 challenger promotion 不通过。**
`legacy + fixed` 继续作为唯一冻结冠军，PCGrad 与 GradNorm-4 仅保留为可审计研究 artifact。

## 10. Artifact 索引

- 诊断报告：`/Users/Zhuanz/PycharmProjects/PythonProject/runs/phase1e-diagnostics-real/diagnostic_report.json`
- PCGrad e3 报告：`/Users/Zhuanz/CLionProjects/quant-backtester-cpp/runs/phase1e-pcgrad-real-e3/challenger_report.json`
- GradNorm-4 e3 报告：`/Users/Zhuanz/CLionProjects/quant-backtester-cpp/runs/phase1e-gradnorm4-real-e3/challenger_report.json`
- PCGrad e4 报告：`/Users/Zhuanz/CLionProjects/quant-backtester-cpp/runs/phase1e-pcgrad-real-e4/challenger_report.json`
- GradNorm-4 e4 报告：`/Users/Zhuanz/CLionProjects/quant-backtester-cpp/runs/phase1e-gradnorm4-real-e4/challenger_report.json`
- Phase 1D 基线：`/Users/Zhuanz/CLionProjects/quant-backtester-cpp/runs/phase1d-real-baseline/phase1d_real_baseline_report.json`
- 数据审计：`/Users/Zhuanz/PycharmProjects/PythonProject/data/research/phase1e_pit_120_audit.json`
- Proxy 回放工具：`/Users/Zhuanz/CLionProjects/quant-backtester-cpp/tools/run_cpp_proxy_replay_cvar.py`
